...
- The Shot X/Y and Chip X/Y Quick dropdowns are multi-select controls: clicking an item must keep the dropdown open, retain existing rows, and expose every selected option through `aria-selected=true` plus the selected style. Their list viewports must not be capped at two rows. Picker right-click clear must clear both the selected chips and the Shot/Chip X/Y rows. `Chip 범위 선택` and `Radius 범위 선택` are mutually exclusive; Radius renders one `Radius(mm)` axis and selects the annulus derived from layout chip-center coordinates. The `coordinate-selection-cells` phase must cover these DOM and selected-chip outcomes without using only viewer state flags.
