...
- Shot boundary/selection 검증은 P001 한 제품만으로 PASS 처리하지 않는다. 같은 wafer positions 위에 synthetic layout products를 만들어 `2×3`, `4×6`, `5×4`, `3×4` nonzero slot origin, `6×4` nonzero slot origin을 모두 검사해야 한다. Origin Shot `(0,0)`이 partial인 제품에서도 frontend `getShotCompositeGridShape()`는 layout FULL Shot 또는 최대 layout group에서 canonical shape를 가져와야 하며, boundary width/height, raw slot modulo, selected Shot chip expansion이 모두 일치해야 한다.
