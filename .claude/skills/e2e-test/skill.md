...
| P1 | Composite/Measure fixed 12-BIN SYSTEMATIC grouping, bottom legend order, and filtered render | `scripts/e2e_chunk1.js` `systematic-bin-group` |
...
| P1 | 선택 Chip/Shot 영역 Composite Map 및 결과 positions 정합성, Shot chip 수/선택영역 crop, 좌표/grid 선택 Good/Bad/Yield summary, 필터된 chip 선택 차단/export, 선택 crop MY LOT/Label 연결 메뉴 | `scripts/e2e_chunk2.js` `coordinate-selection-cells`, `selected-region-composite`, `selected-region-export` |
...
- `systematic-bin-group`는 Composite와 Measure 각각에 `285,286,287,288,290,291,300,385,386,388,389,390`만 들어가는지, Composite/Measure chooser에서는 SYSTEMATIC이 NORMAL/INVALID보다 먼저 표시되는지, 단일/그리드 bottom legend에서도 `Systematic`, `Normal`, `Invalid` 순서인지, filtered render URL, 실제 `mode=systematic` API 응답과 matched chip 수를 함께 확인한다. `BIN336` 같은 비계약 숫자는 SYSTEMATIC에 포함되면 안 된다.
...
- `scripts/e2e_chunk2.js`의 `selected-region-composite`는 실제 P001 fixture에서 단일 Shot, 다중 Chip, 동일 형상 Shot 4/5 두 개, 그리고 현재 layout에서 chip 수가 canonical보다 적은 partial Shot을 검사한다. UI `selected_shot_groups` payload는 각 chip에 `slot_x/slot_y`를 포함해야 한다. Partial Shot Composite 결과는 canonical `4×6` canvas와 동일한 chip cell 크기를 유지해야 하며, 보이는 chip 수만 positions에 남겨야 한다. 선택하지 않은 canonical slot 중심은 palette index 23 흰색이어야 하고, 선택 slot만 selected mask quantile 색으로 렌더되어야 한다. 결과에는 `selection_grade_pixel_counts`, `selection_top_grades`, `selection_chip_inner_pixels`가 있어 상위 grade가 실제로 존재하는지 확인 가능해야 한다.
- Shot boundary/selection 검증은 P001 한 제품만으로 PASS 처리하지 않는다. Focused guard `scripts/e2e_shot_100_products_guard.js`는 fail-map 회전 코드 `5/7/3/0`, chip grid 시작값 약 30, 서로 다른 wafer/shot/chip 구조 100개 synthetic layout product를 만들어 검사해야 한다. Origin Shot `(0,0)`이 partial인 제품에서도 frontend `getShotCompositeGridShape()`는 layout FULL Shot 또는 최대 layout group에서 canonical shape를 가져와야 하며, partial/edge Shot 경계는 display slot 기준 full-size width/height를 유지해야 한다. raw slot modulo와 selected Shot chip expansion도 모두 일치해야 한다.
- Shot 버튼을 이미지 표시 직후 먼저 눌러도 positions/layout 적용 뒤 `shotBoundaryVisible=true`가 유지되어야 한다. `loadPositions()`는 단일 이미지 로딩 경로에서 annotations를 기다리지 않고 Shot layout을 먼저 적용해야 하며, `/api/chip-annotations`를 지연시킨 focused browser guard에서 Shot groups와 boundary pixels가 annotation 응답보다 먼저 준비되어야 한다.
- `layout-chip-coordinates`는 단일 보기 좌표 box가 `Chip(Grid)`, `Chip(Pos)`, `Radious`, `Shot(Grid)` 순서이고 Pos/Radius가 소수 1자리인지 확인한다. TSV export는 `CHIP_COORD_X(mm)`, `CHIP_COORD_Y(mm)`를 별도 컬럼으로 확인한다. Shot 버튼의 화면 경계는 layout Shot `(0, 0)` full Shot을 기준으로 shape/방향/cell size를 정해야 한다. full Shot은 실제 로드된 chip rect union과 일치하고, partial/edge Shot은 edge 자체의 rect 크기·방향을 쓰지 않고 0,0 기준 full Shot 외곽 크기를 유지해야 한다.
...
- BUG-29: Parquet가 integer 의미 컬럼을 float64로 반환해 문자열 1.0 변환 실패가 발생하지 않는지 확인한다. full Shot 화면 경계와 선택 외곽선은 실제 로드된 chip rect union과 일치하고, partial/edge Shot 화면 경계는 0,0 full Shot 기준 크기/방향을 유지해야 한다. 선택색은 실제 존재하는 Chip만 채운다. layout-chip-coordinates는 모든 partial group의 경계 존재, full Shot chip rect union 일치, partial/edge Shot의 0,0 기준 full-size 경계, hover/선택 geometry 일치, 실제 Chip pixel 변화, Chip(Grid)=x_abs/y_abs를 확인한다.
...
- `layout-chip-coordinates` must use the real P001 single-image flow and verify that the reference Shot is layout `(0, 0)`. Full Shot boundaries and Shot hover/selection outlines must match the union of loaded chip rects. Partial/edge Shot boundaries must not infer shape from edge chips; they must keep the same width/height and orientation as the `(0,0)` full Shot while selection fill still touches only existing chips. Shot(Grid), Chip X/Y real-time selection, and Shot picker slots use the same display slot transform. The picker must match the user's on-wafer orientation while numbering its cells bottom-left to right, then upward.
- Layout row duplicates are allowed only when the same `process_id + chip_x_pos + chip_y_pos` has the same geometry and differs by zone metadata. The frontend must collapse these rows before building Shot groups; if geometry differs, the console warning `duplicate chip rows collapsed` must report `conflictingDuplicateKeys > 0`.
- `chip_center_x_pos/y_pos` values from layout are already millimetres. Chip(Pos), Radius, quick selection, range selection, and TSV export must use those raw values without dividing by 1000.
...
- Legend filter selection guard: with a Grade filter active and with `SYSTEMATIC` bottom filter active, a hidden chip coordinate must not be selected by coordinate rows, hover, drag, shot, or chip selection paths. The same phase must prove one hidden coordinate yields selected count 0 while one visible coordinate yields selected count 1. Shot selection must show `Chip 48`, `Shot Yld`, and `Shot Pos Yld` in `#chip-coordinate-select-summary`. Grid multi-image selection must show `#selected-grid-yield-summary` before any Composite action.
...
- Select the smallest partial Shot. Its visible outline keeps the `(0,0)` full Shot size/orientation, the existing Chip center must change to the selection RGBA, and a nearby non-chip/empty-slot point must remain byte-for-byte unchanged.

#### BUG-34: Selected Shot/Chip Composite speed and display path guard
- General Wafer Composite uses the full-image batch/numba accumulator, while selected Shot/Chip Composite uses selected-rect accumulation. On numba-enabled servers, `selected-region-composite` should require response `numba.accumulator=selected_region_numba`.
- Composite result display must stay on the same path as normal images: grid cells request `/api/thumbnail?path=...&size=512`, and double-click single view requests `/api/image?path=...&level=...`. Do not pass by using a composite-only display bypass.
- The same guard should record Shot Composite processing time against the local baseline, output dimensions, positions count, result thumbnail requests, and single-view image requests.
- Shot Composite border width is measured on the generated original PNG, not only the displayed thumbnail. Internal chip boundaries and outer boundaries should both be total 3px by default. The guard must fail if internal shared chip edges become 6px because each chip painted its own 3px border.

#### BUG-35: Single-image tab plus-button independence guard
- Repro path: load one PW/P001 single image, click `#page-add-btn`, then load a different PW/P001 single image in the new tab.
