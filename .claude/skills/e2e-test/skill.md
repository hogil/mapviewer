...
- `systematic-bin-group`는 Composite와 Measure 각각에 `285,286,287,288,290,291,300,385,386,388,389,390`만 들어가는지, Composite/Measure chooser에서는 SYSTEMATIC이 NORMAL/INVALID보다 먼저 표시되는지, 단일/그리드 bottom legend에서는 `Normal`, `Invalid`, `Systematic` 순서인지, filtered render URL, 실제 `mode=systematic` API 응답과 matched chip 수를 함께 확인한다. `BIN336` 같은 비계약 숫자는 SYSTEMATIC에 포함되면 안 된다.
