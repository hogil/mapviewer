...
- BUG-29: Parquet가 integer 의미 컬럼을 float64로 반환해 문자열 1.0 변환 실패가 발생하지 않는지 확인한다. Shot 화면 경계와 선택 외곽선은 실제 로드된 chip rect union과 일치하고, 선택색은 실제 존재하는 Chip만 채운다. layout-chip-coordinates는 모든 partial group의 경계 존재, chip rect union과 경계 일치, hover/선택 geometry 일치, 실제 Chip pixel 변화, Chip(Grid)=x_abs/y_abs를 확인한다.
...
- `layout-chip-coordinates` must use the real P001 single-image flow and verify that the on-screen Shot boundary and Shot hover/selection outline match the union of the loaded chip rects in the same layout Shot group. Partial groups must never disappear or extend into areas with no loaded chips. Shot(Grid), Chip X/Y real-time selection, and Shot picker slots still use layout coordinates. The picker must use the same screen Y direction as Chip X/Y selection while numbering its cells bottom-left to right, then upward.
...
- Select the smallest partial Shot. Its visible outline follows the existing chip rect union, the existing Chip center must change to the selection RGBA, and a nearby non-chip point must remain byte-for-byte unchanged.
