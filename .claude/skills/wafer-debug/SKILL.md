...
- 수정 패턴: top-level은 `선택 ▸` 하나, submenu 안에 `Coord 선택`/`Shot 선택`/`Chip 선택`/`Wafer 선택`을 둔다. Wafer selection이 grid 기본 source이고 Coord는 이를 따른다. Direct Shot/Chip은 Coord와 Shot boundary 버튼을 무시하고 클릭한 wafer 내부에서 선택하며, 클릭한 wafer를 grid selected scope에 넣고 thumbnail overlay는 main annotator `selectedColor`를 쓴다. Grid source가 있으면 세 Composite 항목은 항상 보이고 순서는 `선택 ▸`, `Shot Composite`, `Shot Composite W to W`, `Chip Composite`여야 한다.
- E2E 신호: `selected-region-composite`는 Coord 전 context menu에서 `선택 ▸` submenu 구조, Coord/direct item이 submenu에만 존재, 세 Composite item visible/order, direct Shot/Chip 후 `gridShotBoundaryVisible=false`, target wafer selected wrap/source와 coordinate overlay selected chip count를 확인한다.
