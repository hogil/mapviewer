...
- 수정 패턴: requested Shot Composite는 `selectionMode='chip'`이어도 selected chip을 실제 Shot group별로 묶어 `selected_shot_groups`를 만든다. Shot Position 선택이면 각 group에는 선택 slot chip만 들어가야 한다. selected Shot canonical canvas의 기본 index는 0이고, grade accumulation과 result positions는 선택 chip만 사용한다. grid page state에는 selected coords, targetPath, pending source wafer를 저장하고 wafer 복귀 시 positions/layout 준비 후 chip selection과 grid coord overlay를 복원한다.
