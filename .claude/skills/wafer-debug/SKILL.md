...
- E2E 신호: `selected-region-composite`는 Coord 전 context menu에서 `Wafer/Shot/Chip 선택 ▸` submenu 구조와 Composite 아래 위치, Coord/direct item이 submenu에만 존재, 세 Composite item visible/order, direct Shot/Chip 후 `gridShotBoundaryVisible=false`, target wafer selected wrap/source와 coordinate overlay selected chip count를 확인한다.
