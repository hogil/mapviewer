...
- 평가: 서버 시작 직후 브라우저 `GET /`가 `AUTO_LOGIN=1`일 때 `/saml/login`으로 즉시 넘어가야 하고, `/saml/login`은 explorer/full-app/composite 준비를 기다리지 않고 IdP redirect를 시작해야 한다. 반복 로그인 시 `/saml/login`/`/saml/acs` 로그의 PID와 runtime import 성공 여부가 일관되어야 한다.
