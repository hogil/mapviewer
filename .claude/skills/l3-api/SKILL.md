...
## 2026-09-11 감사 회귀: 저장/API 경계
- `api/main.py`, `api/full_app.py`: root/raw-log static mount 금지, color-legends만 bootstrap에서 명시 제공, JS/CSS resolved asset boundary 검사. HTTPException은 그대로 전달한다.
- `api/full_app.py`: class/label batch 삭제도 권한 preflight; legacy labels는 classify write/delete에 위임; users/search를 users/{username}보다 먼저 등록하고 get_current_user는 기존 URL handoff resolver를 공유한다. SAML cookie를 새로 추가하지 않는다.
- Label Explorer basename-only delete와 legacy classification-path delete의 LABEL_WRITE 기준은 원본 이미지 폴더다. classification 상대 경로를 원본으로 resolve한 뒤 검사하며 basename parent '.'로 검사하지 않는다. check_audit_api.py의 source-folder-only grant 회귀를 유지한다.
- `api/my_lot.py`: '.'/'..'/trailing-dot 및 symlink 경계 이탈 거부, image+positions rename의 preflight/rollback, batch copy/rename 동일 lock. lock 대기 API는 executor/thread에서 실행한다.
- 검증: scripts/check_audit_api.py, scripts/check_audit_mylot.py; e2e_chunk1 audit-regressions에서 raw static404/public assets200/missing image404/invalid group400/rename 후 실제 canvas+chip count 보존.
- Composite NPZ 저장 완료는 작업 완료보다 앞서야 한다. 지연 daemon writer가 다음 cleanup과 경합하던 회귀는 check_composite_persistence.py와 Chunk2 즉시 Subset/positions로 검사한다. NPZ 교체는 고유 temp 및 짧은 publication lock만 사용하며 실패 때 기존 캐시를 지우지 않는다. recolor도 invalidate+render+persist를 COMPOSITE_EXECUTOR에서 실행하고 저장 실패를 성공으로 바꾸지 않는다. recolor JSON은 객체여야 하며 출력 경로는 문자열 prefix가 아닌 resolved parent 경계로 검사한다.
- 확장 동시성: 동일 사용자 Composite generation/cleanup/recolor/subset은 `_composite_output_lock`을 worker에서 획득한다. 오래된 writer가 최신1칩 결과를833칩으로 덮지 않는지 `composite-same-user-overlap`으로 확인하며 다른 사용자 병렬성은 유지한다.
- Windows mutable PNG의 thumbnail/pyramid뿐 아니라 chip-grade/palette numpy·원본 FileResponse도 삭제를 막았다. `_mutable_image_guard/_mutable_image_snapshot`으로 classification/classification_chips/my-lot source 읽기만 mutation과 직렬화하고 decode와 raw GET은 bytes를 사용한다. class mutation API는 thread에서 대기하며 MY LOT 기존lock을 재사용한다. HEAD는 full snapshot을 읽지 않는다. `check_mutable_image_races.py`와 active-class-delete/save-rename-delete가 실제held reader·event loop·삭제후부재를 검사한다.
- MY LOT batch의 없는 paths도 errors/error_count에 합치고 모든누락은각입력실패를반환한다. 삭제중get_image의자기source FileNotFoundError만404, 다른내부파일실패는500으로유지한다. `check_extended_batch.py`와 storage-partial-accounting으로 검증한다.
