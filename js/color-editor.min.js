...
export class ColorSchemeEditor {
    ...
    async close() {
        ...
        // 🔥 취소 시: 미리보기 적용된 경우만 원래 색상으로 복원 + 리로드
        try {
            if (this._previewApplied && this.originalSchemeData && this.currentSchemeName) {
                // 미리보기가 적용된 경우에만 복원 수행
                if (this.viewer.colorLegends) {
                ...
                // 이미지/그리드 리로드 (미리보기 복원)
                if (this.viewer.gridMode) {
                    const currentImages = this.viewer.currentGridImages?.length > 0
                        ? this.viewer.currentGridImages
                        : (this.viewer.selectedImages?.length > 0 ? this.viewer.selectedImages : []);
                ...
                // 🔥 미리보기 없이 취소 — 리로드 불필요, __preview_ 정리만
                await this.cleanupPreviewSchemeArtifacts();
                this.viewer._previewSchemeOverride = null;
            }
        } catch (error) {
        ...
        this._previewApplied = false; // 🔥 플래그 리셋
        ...
        try {
            ...
            if (result.success) {
                ...
                if (this.viewer) {
                    ...
                    if (this.viewer.gridMode) {
                        // 그리드 모드: 현재 그리드 유지 (currentGridImages 우선, selectedImages fallback)
                        const currentImages = this.viewer.currentGridImages?.length > 0
                            ? this.viewer.currentGridImages
                            : (this.viewer.selectedImages?.length > 0 ? this.viewer.selectedImages : []);
                        if (currentImages.length > 0) {
                    ...
                    // 🔥 Composite 모드: Grade 색 변경 → recolor → 그리드 전체 재로드
                    if (this.viewer.isCompositeMode && this.viewer.compositeSession?.outputDir) {
                        try {
                            const outputDir = this.viewer.compositeSession.outputDir;
                            const recolorRes = await fetch('/api/composite-recolor', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ output_dir: outputDir }),
                            });
                            if (recolorRes.ok) {
                                const recolorData = await recolorRes.json();
                                if (Array.isArray(recolorData.sum_maps) && recolorData.sum_maps.length) {
                                    this.viewer.compositeSession.sumMaps = recolorData.sum_maps.map(e => ({ ...e }));
                                }
                            }
                            // 🔥 recolor 완료 후 composite 폴더 전체 재로드
                            await this.viewer.loadImagesInFolderAndShowGrid(outputDir);
                        } catch (recolorErr) {
                            console.warn('[ColorEditor] composite recolor after grade save:', recolorErr);
                        }
                    }
    ...
    async _handleApplyGradient(tabType) {
        ...
        try {
            ...
            if (result.colors) {
                ...
                // 🔥 Composite 모드: 서버에서 새 색상으로 recolor → 그리드 전체 재로드
                // Composite 탭 색상은 Grade + Measure 모두에 적용됨
                if (this.viewer.isCompositeMode && this.viewer.compositeSession) {
                    try {
                        const outputDir = this.viewer.compositeSession.outputDir;
                        if (outputDir) {
                            // 1) Grade Composite recolor (sum_maps)
                            const recolorRes = await fetch('/api/composite-recolor', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ output_dir: outputDir, colors: colorsArray }),
                            }).catch(() => null);
                            if (recolorRes?.ok) {
                                const recolorData = await recolorRes.json();
                                if (Array.isArray(recolorData.sum_maps) && recolorData.sum_maps.length) {
                            ...
                            // 2) Measure Composite recolor (F/Q/BIN 이미지들)
                            const measureResults = (this.viewer.compositeSession.measureResults || [])
                                .filter(r => !r.is_grade_composite && r.filename);
                            if (measureResults.length > 0) {
                                await Promise.all(measureResults.map(async (r) => {
                                    try {
                                        await fetch('/api/measure-composite-recolor', {
                                            method: 'POST',
                                            headers: { 'Content-Type': 'application/json' },
                                            body: JSON.stringify({
                                                output_dir: r.output_dir || outputDir,
                            ...
                            // 🔥 서버 recolor 완료 후 composite 폴더 전체 재로드 (Grade + Measure 모두 갱신)
                            await this.viewer.loadImagesInFolderAndShowGrid(outputDir);
                        }
                    } catch (recolorErr) {
                        console.warn('[ColorEditor] composite recolor failed:', recolorErr);
                    }
                } else if (this.viewer.gridMode) {
                    if (typeof this.viewer.refreshGridThumbnailsWithCurrentParams === 'function') {
                        this.viewer.refreshGridThumbnailsWithCurrentParams();
