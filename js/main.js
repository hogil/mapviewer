...
class WaferMapViewer {
    ...
    async loadImage(path, fromLabelExplorer = false, loadVersion = null, forceReload = false, options = {}) {
        const { preserveBottomSelection = false } = options || {};
    ...
    startGridThumbnailLoad(img) {
        ...
        const onLoad = () => {
            // 🔥 placeholder 이미지 로드는 성공으로 간주하지 않음 (취소된 이미지 재로드 가능하도록)
            if (img.src === GRID_THUMB_PLACEHOLDER || !img.dataset.src) {
                finalize(false);
                return;
            }
            finalize(true);
        ...
        const onError = () => {
            ...
            if (retryCount < 3 && img.dataset?.src) {
                ...
                console.warn(`이미지 로드 실패 (재시도 ${retryCount}회): ${img.dataset.src}`);
                delete img.dataset.retryCount; // 재시도 카운터 초기화
                finalize(false);
            }
        };
        ...
        // 🔥 이미지가 이미 로드된 경우 (캐시된 경우) 즉시 완료 처리
        if (img.complete && img.naturalWidth > 0) {
            // 🔥 placeholder 이미지는 성공으로 간주하지 않음
            if (img.src === GRID_THUMB_PLACEHOLDER || !img.dataset.src) {
                finalize(false);
            } else {
                finalize(true);
    ...
    resolveLabelExplorerImagePath(key) {
        ...
        if (
            ...
            return this.ensureFolderPrefix(normalizedKey);
        }
    ...
    async applyGradeFilter() {
        if (this.gridMode) {
            const currentImages = (this.currentGridImages && this.currentGridImages.length > 0)
                ? this.currentGridImages
                : this.selectedImages;
            if (currentImages && currentImages.length > 0) {
                ...
                await this.showGrid(currentImages, true);
            }
            return;
        }
        ...
        await this.loadImage(this.selectedImagePath, false, null, true, { preserveBottomSelection: true });
    }
