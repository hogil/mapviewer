...
class WaferMapViewer {
    ...
    bindFilterEvents() {
        ...
        if (this.dom.personalizedColorButton) {
            console.log('✅ [DEBUG] personalizedColorButton 발견됨:', this.dom.personalizedColorButton);
            this.dom.personalizedColorButton.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('🔘 [DEBUG] 개인색 설정 버튼 클릭됨');
            ...
            setTimeout(() => {
                ...
                if (btn) {
                    ...
                    btn.addEventListener('click', (e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('🔘 [DEBUG] 개인색 설정 버튼 클릭됨 (지연 로딩)');
                        this.onPersonalColorButtonClick();
                    });
                }
            }, 100);
    ...
    async setClassMode(mode) {
        ...
        if (this.labelManager && typeof this.labelManager.refreshAll === 'function') {
            this.labelManager.refreshAll()
                .then(() => console.log(`🔍 [CLASS_MODE] LabelManager 새로고침 완료 (mode=${normalized})`))
                .catch((error) => console.error('LabelManager refresh 실패:', error));
        }
        ...
        this.refreshLabelExplorer()
            ...
            .catch(err => console.error('Label Explorer 새로고침 실패:', err));
    }
    ...
    buildClassificationPath(subPath = '', { includePrefix = true } = {}) {
        ...
        if (includePrefix && this.currentFolderPrefix) {
    ...
    onPersonalColorButtonClick() {
        console.log('🎨 [DEBUG] onPersonalColorButtonClick 호출됨');
        if (!this.colorEditor) {
            console.error('❌ [DEBUG] colorEditor가 초기화되지 않았습니다.');
            this.showToast?.('색상 편집기를 초기화하지 못했습니다.', 1800);
            return;
        }
        console.log('✅ [DEBUG] colorEditor.open() 호출');
