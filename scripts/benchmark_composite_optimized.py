"""
Composite Map 최적화 버전 성능 벤치마크
현재 적용된 최적화:
- pyvips 사용 (memory=True)
- 병렬 저장 (4개 워커)
- PNG compress_level=0
- 배열 Contiguous 보장
- 워커 수 최적화 (cpu_count * 2, 최대 16)
- 배치 크기 증가 (기본 4)
"""
import sys
import time
...
def main():
    ...
    # 최적화 정보 출력
    print("적용된 최적화:")
    print("  ✓ pyvips 사용 (memory=True)")
    print("  ✓ 병렬 저장 (4개 워커)")
    print("  ✓ PNG compress_level=0")
    print("  ✓ 배열 Contiguous 보장")
    print("  ✓ 워커 수 최적화 (cpu_count * 2, 최대 16)")
    print("  ✓ 배치 크기 증가 (기본 4)")
    print()
    ...
    try:
        ...
        print(f"  - 이미지 로딩: {result.get('loading_time', 'N/A')}초" if 'loading_time' in result else "")
        print(f"  - 저장 시간: {result.get('save_time', 'N/A')}초" if 'save_time' in result else "")
        ...
        if 'loading_time' in result and 'save_time' in result:
            print(f"  - 로딩: {result['loading_time']:.2f}초 ({((10.53 - result['loading_time']) / 10.53 * 100):.1f}% 개선)")
            print(f"  - 저장: {result['save_time']:.2f}초 ({((5.92 - result['save_time']) / 5.92 * 100):.1f}% 개선)")
        print("=" * 70)
