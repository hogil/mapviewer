...
# composite_map_v2.py 임포트 (사용자가 제공한 최적화 버전)
try:
    from api.composite_map_v2 import create_composite_heatmaps_optimized
    V2_AVAILABLE = True
except ImportError:
    print("[ERROR] composite_map_v2.py를 찾을 수 없습니다.")
    print("경로: api/composite_map_v2.py")
    V2_AVAILABLE = False

from api.config import IMAGES_ROOT

def benchmark_composite_map_v2(image_folder: str, num_images: int = 10):
    """
    Composite Map V2 생성 성능 평가
    ...
    print("=" * 80)
    print("Composite Map V2 (최적화 버전) 성능 평가")
    print("=" * 80)
    ...
    # 성능 측정
    print("\n" + "=" * 80)
    print("[START] Composite Map V2 생성 시작...")
    print("=" * 80)
    ...
    try:
        result = create_composite_heatmaps_optimized(
            ...
            max_workers=4,
            batch_size=2,
            use_streaming_median=False  # 표준 median 사용
        )
        ...
        # 결과 출력
        print("\n" + "=" * 80)
        print("[RESULT] 성능 평가 결과 (V2 최적화 버전)")
        print("=" * 80)
        ...
        if 'loading_time' in result:
            print(f"[TIME] 로딩 시간: {result['loading_time']:.2f}초")
        if 'save_time' in result:
            print(f"[TIME] 저장 시간: {result['save_time']:.2f}초")
        ...
        # V1과 비교 (이전 결과)
        print("\n" + "=" * 80)
        print("[COMPARE] V1 vs V2 비교 (이전 결과 기준)")
        print("=" * 80)
        v1_time = 14.46  # 이전 평가 결과
        improvement = ((v1_time - total_time) / v1_time) * 100
        speedup = v1_time / total_time if total_time > 0 else 0
        print(f"[COMPARE] V1 (기존): {v1_time:.2f}초")
        print(f"[COMPARE] V2 (최적화): {total_time:.2f}초")
        print(f"[COMPARE] 개선율: {improvement:.1f}%")
        print(f"[COMPARE] 속도 향상: {speedup:.2f}배")
...
if __name__ == "__main__":
    ...
    benchmark_composite_map_v2(image_folder, num_images)
