...
(async () => {
  ...
  await record('coordinate-selection-cells', '연동 Shot/Chip/Chip ID 셀 / Shift marquee / wildcard / Chip·Radius AND 범위 / Chip Pos', async () => {
    ...
    const mapButtonState = await page.evaluate(() => {
      const shotMap = document.querySelector('[data-coordinate-list-map="shot"]');
      const chipMap = document.querySelector('[data-coordinate-list-map="chip"]');
      const chipIdMap = document.querySelector('[data-coordinate-list-map="chipId"]');
      return {
        shotExists: !!shotMap,
        chipExists: !!chipMap,
        chipIdMapExists: !!chipIdMap,
      };
    });
    expect(mapButtonState.shotExists && mapButtonState.chipExists && mapButtonState.chipIdMapExists === false,
      `coordinate map buttons exist=${JSON.stringify(mapButtonState)}`);

    await page.locator('[data-coordinate-list-map="shot"]').click();
    await page.waitForFunction(
    ...
    const mapPanelStructure = await page.evaluate(() => {
      const modal = document.getElementById('coordinate-map-select-modal');
      const panel = modal?.querySelector('.coordinate-map-select-modal-content');
      return {
        overlayPointerEvents: modal ? getComputedStyle(modal).pointerEvents : '',
        overlayBackground: modal ? getComputedStyle(modal).backgroundColor : '',
    ...
    expect(mapPanelStructure.overlayPointerEvents === 'none' &&
      /rgba\(0,\s*0,\s*0,\s*0\)/.test(mapPanelStructure.overlayBackground) &&
    ...
    const shotMapPoint = await page.evaluate(() => {
      ...
      const canvas = document.getElementById('coordinate-map-select-overlay');
      const group = Array.from(annotator?.shotBoundaryGroups?.values?.() || [])[0];
      const chip = group?.chips?.find((candidate) => annotator.isChipSelectable?.(candidate) !== false) || group?.chips?.[0];
      const rect = chip?.rect;
    ...
    await page.locator('[data-coordinate-list-map="chip"]').click();
    await page.waitForFunction(
      () => getComputedStyle(document.getElementById('coordinate-map-select-modal')).display !== 'none' &&
    ...
    const chipMapPoint = await page.evaluate(() => {
      const v = window.viewer;
      const annotator = v?.coordinateMapSelectionAnnotator;
      const canvas = document.getElementById('coordinate-map-select-overlay');
      const chip = annotator?.chips?.find((candidate) => annotator.isChipSelectable?.(candidate) !== false);
      const rect = chip?.rect;
      const transform = v?.coordinateMapSelectionViewer?.transform;
      const canvasRect = canvas?.getBoundingClientRect();
      if (!rect || !transform || !canvasRect) return null;
    ...
    expect(!!chipMapPoint, `chip map point=${JSON.stringify(chipMapPoint)}`);
    await page.mouse.click(chipMapPoint.x, chipMapPoint.y);
    await page.waitForFunction(
      () => [...document.querySelectorAll('#chip-coordinate-select-chip-tbody input[data-coordinate-row]')]
        .map((input) => input.value)
        .filter(Boolean).length === 2 &&
        window.viewer?.chipAnnotator?.selectionMode === 'chip',
      null,
    ...
    const chipMapSelection = await page.evaluate(() => ({
      activeList: window.viewer?.coordinateSelectionActiveList || '',
      mapList: window.viewer?.coordinateSelectionMapMode?.listName || '',
      pressed: document.querySelector('[data-coordinate-list-map="chip"]')?.getAttribute('aria-pressed') || '',
      modalSelected: window.viewer?.coordinateMapSelectionAnnotator?.selectedChips?.size || 0,
      mainSelected: window.viewer?.chipAnnotator?.selectedChips?.size || 0,
      chipRows: [...document.querySelectorAll('#chip-coordinate-select-chip-tbody input[data-coordinate-row]')]
        .map((input) => input.value)
        .filter(Boolean),
    }));
    expect(chipMapSelection.activeList === 'chip' &&
      chipMapSelection.mapList === 'chip' && chipMapSelection.pressed === 'true' &&
      chipMapSelection.modalSelected === 1 && chipMapSelection.mainSelected === 1 &&
      chipMapSelection.chipRows.length === 2,
      `chip map selection=${JSON.stringify(chipMapSelection)}`);
    await page.locator('#coordinate-map-select-clear').click();
    await page.waitForFunction(
      () => (window.viewer?.chipAnnotator?.selectedChips?.size || 0) === 0,
      null,
      { timeout: 10000 }
    );
    await page.locator('#coordinate-map-select-done').click();
    const mapCleanup = await page.evaluate(() => ({
      modalHidden: getComputedStyle(document.getElementById('coordinate-map-select-modal')).display === 'none',
      selectedAfterCleanup: window.viewer?.chipAnnotator?.selectedChips?.size || 0,
      mapAfterCleanup: window.viewer?.coordinateSelectionMapMode?.listName || '',
    }));
    expect(mapCleanup.modalHidden &&
      mapCleanup.selectedAfterCleanup === 0 &&
      mapCleanup.mapAfterCleanup === '',
      `coordinate map cleanup=${JSON.stringify(mapCleanup)}`);
    ...
    const rangeStructure = await page.evaluate(() => ({
      ...
      tabs: document.querySelectorAll('#chip-coordinate-select-range-tabs [data-coordinate-range-tab]').length,
      fieldTabs: document.querySelectorAll('#chip-coordinate-select-range-fields [data-coordinate-range-tab]').length,
      activeTabs: document.querySelectorAll('#chip-coordinate-select-range-tabs [data-coordinate-range-tab].is-active').length,
      tabsAfterAdd: document.getElementById('chip-coordinate-select-range-tabs')?.previousElementSibling?.id === 'chip-coordinate-select-range-add',
    ...
    expect(rangeStructure.addButton && rangeStructure.groups === 1 && rangeStructure.sets === 1 &&
      rangeStructure.tabs === 1 && rangeStructure.fieldTabs === 0 && rangeStructure.activeTabs === 1 &&
      rangeStructure.tabsAfterAdd && rangeStructure.visibleSets === 1 &&
    ...
    const setRangeNumber = async (setIndex, kind, axis, bound, value) => {
      await page.locator('#chip-coordinate-select-range-tabs [data-coordinate-range-tab]').nth(setIndex).click();
    ...
    const reversedRangeState = await page.evaluate(() => {
      ...
      return {
        ...
        left: slider.style.getPropertyValue('--range-left'),
        right: slider.style.getPropertyValue('--range-right'),
      };
    });
    ...
    const radiusRangeControls = await page.evaluate(() => ({
      setCount: document.querySelectorAll('#chip-coordinate-select-range-fields .coordinate-select-range-set').length,
      tabCount: document.querySelectorAll('#chip-coordinate-select-range-tabs [data-coordinate-range-tab]').length,
      fieldTabCount: document.querySelectorAll('#chip-coordinate-select-range-fields [data-coordinate-range-tab]').length,
    ...
    expect(radiusRangeControls.setCount === 1 &&
      radiusRangeControls.tabCount === 1 && radiusRangeControls.fieldTabCount === 0 &&
      radiusRangeControls.visibleSetCount === 1 &&
    ...
    expect(rangeOrSelection.selected === 2 && rangeOrSelection.setCount === 2 &&
      rangeOrSelection.tabCount === 2 && rangeOrSelection.fieldTabCount === 0 &&
      rangeOrSelection.activeTabs === 1 && rangeOrSelection.visibleSetCount === 1 &&
    ...
    await page.waitForFunction(
      ({ x, y, shots }) => {
        const annotator = window.viewer?.chipAnnotator;
        const selected = [...(annotator?.selectedChips || [])];
        return selected.length === 1 && selected.every((index) => {
          const chip = annotator.chips?.[index];
          const layout = annotator.getLayoutRowForChip?.(chip);
          return Math.abs(Number(layout?.chip_center_x_pos) - Number(x)) < 0.001 &&
            Math.abs(Number(layout?.chip_center_y_pos) - Number(y)) < 0.001 &&
            shots.some((shot) => Number(shot.x) === Number(layout?.shot_x_pos) &&
              Number(shot.y) === Number(layout?.shot_y_pos));
        });
      },
      { x: selectionTarget.pos.x, y: selectionTarget.pos.y, shots: selectionTarget.shotRows },
      { timeout: 10000 }
    );
    const rangeAndSelection = await page.evaluate(() => ({
      selected: window.viewer?.chipAnnotator?.selectedChips?.size || 0,
      activeList: window.viewer?.coordinateSelectionActiveList || '',
      shotRows: [...document.querySelectorAll('#chip-coordinate-select-shot-tbody input[data-coordinate-row]')]
        .map((input) => input.value)
  ...
  await record('selected-region-composite', '선택 Chip/Shot Composite Map 및 결과 positions 정합성', async () => {
    ...
    const gridShotOverlay = await page.evaluate(() => {
      ...
      return {
        ...
        stroke: rendered?.dataset?.shotBoundaryStroke || '',
        dash: rendered?.dataset?.shotBoundaryDash || '',
    ...
    const gridContextShotSelectNoCoordBefore = await page.evaluate(() => ({
      ...
      coordMenuPresent: !!document.getElementById('grid-shot-coordinate-select-open'),
      ...
      directItemOrder: [...document.querySelectorAll('#grid-direct-selection-submenu > .context-menu-item')]
        .map((item) => item.id),
      directActiveItemId: document.querySelector('#grid-direct-selection-submenu > .context-menu-item[data-direct-selection-active="true"]')?.id || '',
      directItemsInSubmenu: ['grid-direct-wafer-select', 'grid-direct-shot-select', 'grid-direct-chip-select']
    ...
    const gridDirectShotPoint = await page.evaluate(async ({ imagePath }) => {
      const v = window.viewer;
      ...
      const group = (data?.groups || []).find((candidate) => Number(candidate.chipCount || 0) > 0);
      const thumbBox = wrap.querySelector('.grid-thumb-imgbox');
      const rect = thumbBox?.getBoundingClientRect?.();
      if (!data?.canvasWidth || !data?.canvasHeight || !group || !rect) return { ok: false, reason: 'missing group' };
    ...
    expect(gridDirectShotPoint.ok, `grid direct shot click point=${JSON.stringify(gridDirectShotPoint)}`);
    await page.mouse.move(gridDirectShotPoint.x, gridDirectShotPoint.y);
    await page.waitForFunction(
      (chipCount) => Array.from(document.querySelectorAll('.grid-coordinate-selection-overlay'))
        .some((canvas) => canvas.dataset.coordinateHoverRendered === 'true' &&
          Number(canvas.dataset.coordinateHoverChipCount || '0') === chipCount) &&
        document.getElementById('coord-chip-coord')?.textContent?.trim() !== '-' &&
        document.getElementById('coord-shot')?.textContent?.trim() !== '-',
      gridDirectShotPoint.chipCount,
      { timeout: 30000 }
    );
    const gridDirectShotHover = await page.evaluate(() => ({
      coordChip: document.getElementById('coord-chip-coord')?.textContent?.trim() || '',
      coordShot: document.getElementById('coord-shot')?.textContent?.trim() || '',
      hoverRendered: Array.from(document.querySelectorAll('.grid-coordinate-selection-overlay'))
        .some((canvas) => canvas.dataset.coordinateHoverRendered === 'true'),
      maxHoverChipCount: Math.max(0, ...Array.from(document.querySelectorAll('.grid-coordinate-selection-overlay'))
        .map((canvas) => Number(canvas.dataset.coordinateHoverChipCount || '0'))),
    }));
    ...
    const gridDirectShotAfter = await page.evaluate(() => ({
    ...
    `grid direct Shot selection should not open Coord or create pending source=${JSON.stringify({ gridDirectShotHover, gridDirectShotAfter })}`);
    const gridDirectSecondShotPoint = await page.evaluate(async ({ imagePath, shotX, shotY }) => {
      const v = window.viewer;
      const pathKey = v?.normalizePath?.(imagePath);
      const wrap = Array.from(document.querySelectorAll('#image-grid .grid-thumb-wrap'))
        .find((candidate) => v?.normalizePath?.(candidate.dataset.path || '') === pathKey);
    ...
    await page.keyboard.down('Control');
    try {
      await page.mouse.click(gridDirectSecondShotPoint.x, gridDirectSecondShotPoint.y);
    } finally {
      await page.keyboard.up('Control');
    }
    await page.waitForFunction(
      ({ chipCount, firstIndex, secondIndex }) => window.viewer?.gridDirectSelectionMode === 'shot' &&
        window.viewer?.gridDirectSelectionSourceSet?.size === 2 &&
        window.viewer?.chipAnnotator?.selectedChips?.size === chipCount &&
        window.viewer?.gridSelectedIdxs?.includes(firstIndex) &&
        window.viewer?.gridSelectedIdxs?.includes(secondIndex) &&
        Array.from(document.querySelectorAll('.grid-coordinate-selection-overlay'))
          .some((canvas) => canvas.dataset.coordinateOverlayRendered === 'true' &&
            Number(canvas.dataset.coordinateSelectedChipCount || '0') === chipCount),
    ...
    expect(gridDirectShotCtrlSecondAfter.mode === 'shot' &&
      ...
      gridDirectShotCtrlSecondAfter.selectedWraps.includes(gridCoordSecond.index) &&
      gridDirectShotCtrlSecondAfter.modalHidden,
      `grid direct Shot Ctrl on another wafer must preserve Shot selection=${JSON.stringify(gridDirectShotCtrlSecondAfter)}`);
    await page.locator('#image-grid .grid-thumb-wrap').nth(target.index).click({ button: 'right' });
    await page.waitForFunction(
    ...
    const gridDirectShotMenuState = await page.evaluate(() => ({
      directShotText: document.getElementById('grid-direct-shot-select')?.textContent?.trim() || '',
      directChipText: document.getElementById('grid-direct-chip-select')?.textContent?.trim() || '',
      directWaferText: document.getElementById('grid-direct-wafer-select')?.textContent?.trim() || '',
      directItemOrder: [...document.querySelectorAll('#grid-direct-selection-submenu > .context-menu-item')]
    ...
    expect(gridDirectShotMenuState.mode === 'shot' &&
      gridDirectShotMenuState.directShotText === '✓ Shot 선택' &&
      gridDirectShotMenuState.directChipText === 'Chip 선택' &&
      gridDirectShotMenuState.directWaferText === 'Wafer 선택' &&
      gridDirectShotMenuState.directActiveItemId === 'grid-direct-shot-select' &&
      gridDirectShotMenuState.directItemOrder.join('|') === 'grid-direct-wafer-select|grid-direct-shot-select|grid-direct-chip-select',
      `grid direct submenu must mark Shot mode=${JSON.stringify(gridDirectShotMenuState)}`);
    ...
    const gridDirectChipSelectedCoord = await page.evaluate(() => {
      const v = window.viewer;
      const selectedIndex = [...(v?.chipAnnotator?.selectedChips || [])][0];
      const chip = v?.chipAnnotator?.chips?.[selectedIndex];
      return {
        xAbs: Number(chip?.x_abs ?? chip?.x),
        yAbs: Number(chip?.y_abs ?? chip?.y),
      };
    });
    expect(Number.isFinite(gridDirectChipSelectedCoord.xAbs) && Number.isFinite(gridDirectChipSelectedCoord.yAbs),
      `grid direct chip selected coord missing=${JSON.stringify(gridDirectChipSelectedCoord)}`);
    const gridDirectSecondChipPoint = await page.evaluate(async ({ imagePath, xAbs, yAbs }) => {
      const v = window.viewer;
      const pathKey = v?.normalizePath?.(imagePath);
      const wrap = Array.from(document.querySelectorAll('#image-grid .grid-thumb-wrap'))
        .find((candidate) => v?.normalizePath?.(candidate.dataset.path || '') === pathKey);
    ...
    await page.keyboard.down('Control');
    try {
      await page.mouse.click(gridDirectSecondChipPoint.x, gridDirectSecondChipPoint.y);
    } finally {
      await page.keyboard.up('Control');
    }
    await page.waitForFunction(
      ({ firstIndex, secondIndex }) => window.viewer?.gridDirectSelectionMode === 'chip' &&
        window.viewer?.gridDirectSelectionSourceSet?.size === 2 &&
        window.viewer?.chipAnnotator?.selectedChips?.size === 1 &&
        window.viewer?.gridSelectedIdxs?.includes(firstIndex) &&
        window.viewer?.gridSelectedIdxs?.includes(secondIndex) &&
    ...
    const gridDirectChipCtrlSecondAfter = await page.evaluate(() => ({
      mode: window.viewer?.gridDirectSelectionMode || '',
      directSourceCount: window.viewer?.gridDirectSelectionSourceSet?.size || 0,
      selectedChips: window.viewer?.chipAnnotator?.selectedChips?.size || 0,
      directSelections: Array.from(window.viewer?.gridDirectSelectionBySource || new Map())
    ...
    expect(gridDirectChipCtrlSecondAfter.mode === 'chip' &&
      ...
      gridDirectChipCtrlSecondAfter.selectedWraps.includes(gridCoordSecond.index) &&
      gridDirectChipCtrlSecondAfter.modalHidden,
      `grid direct Chip Ctrl on another wafer must preserve Chip selection=${JSON.stringify(gridDirectChipCtrlSecondAfter)}`);
    await page.locator('#grid-coordinate-select-open-btn').click();
    await page.waitForFunction(
      () => window.viewer?.gridMode === true &&
        !window.viewer?.viewMode &&
    ...
    const gridContextShotSelectNoCoordAfter = await page.evaluate(() => ({
      ...
      gridMode: window.viewer?.gridMode === true,
      viewMode: window.viewer?.viewMode || null,
      shotOverlay: window.viewer?.gridShotBoundaryVisible === true,
      pendingSourceCount: window.viewer?._pendingGridRegionComposite?.sourceImages?.length || 0,
      selectedPaths: window.viewer?._getGridSelectedImagePaths?.() || [],
      menuHidden: getComputedStyle(document.getElementById('grid-context-menu')).display === 'none',
    }));
    expect(gridContextShotSelectNoCoordBefore.text === '' &&
      !gridContextShotSelectNoCoordBefore.coordMenuPresent &&
      ...
      gridContextShotSelectNoCoordBefore.directActiveItemId === 'grid-direct-wafer-select' &&
      gridContextShotSelectNoCoordBefore.directItemOrder.join('|') ===
        'grid-direct-wafer-select|grid-direct-shot-select|grid-direct-chip-select' &&
      ...
      `grid context Shot select without prior Coord=${JSON.stringify({ gridContextShotSelectNoCoordBefore, gridDirectShotHover, gridDirectShotAfter, gridContextShotSelectNoCoordAfter })}`);
    await page.locator('#chip-coordinate-select-close').click();
    await page.waitForFunction(
      () => getComputedStyle(document.getElementById('chip-coordinate-select-modal')).display === 'none',
      null,
