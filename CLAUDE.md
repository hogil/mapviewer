...
- Later E2E regression: chunk2 `Navigator / Minimap / repeated restore` failed with `explorer count 3000->1` after clicking a Wafer Map Explorer file from a 3000-image grid and returning from single view. Fix: preserve the previous grid return state before `selectedImages=[path]`, prefer the larger saved grid image list on single-view exit, and rehydrate `currentGridImages/selectedImages` during fast grid restore.
- Final verification sessions after the fix: full headless `20260501-084834-e21622fd` passed chunk1/chunk2/chunk3 (`7 + 4 + 6` records), and visible local Playwright chunk3 `20260501-085402-10dfcadc` passed. The visible run proved exactly 10 tabs for wafer/measure/composite/label/mylot grid+single views, preserved grid selections/scroll/explorer selections, kept `composite_cache_v1` absent, and generated a 10-image Composite Map with server-side Numba warmup (`warmup_time=0`, `processingTime=4.89s`, elapsed about 6.7s).
